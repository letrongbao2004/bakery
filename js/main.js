$(document).ready(function () {
  ...
  window.handleLogout = function (e) {
    ...
    showNotification(
      "info",
      "Đã Đăng Xuất",
      "Bạn đã đăng xuất thành công. Hẹn gặp lại!",
    );
    setTimeout(() => {
  ...
  window.addToCart = function (item) {
    ...
    showNotification(
      "success",
      "Đã Thêm Vào Giỏ! 🛒",
      `<strong>${item.name}</strong> đã được thêm vào giỏ hàng.`,
    );
  };
  ...
  if (currentPath === "checkout.html") {
    ...
    function setStep(step) {
      ...
      ["#stepDot1", "#stepDot2", "#stepDot3"].forEach((d, i) => {
        $(d)
          .toggleClass("active", i + 1 === step)
          .toggleClass("done", i + 1 < step);
      });
    ...
    $("#checkoutInfoForm").submit(function (e) {
      ...
      let itemsHtml =
        '<div class="table-responsive"><table class="table table-sm mb-3"><thead class="table-light"><tr><th>Sản phẩm</th><th class="text-end">Thành tiền</th></tr></thead><tbody>';
      let total = 0;
      cart.forEach((i) => {
        let lt = i.price * i.qty;
        total += lt;
        itemsHtml += `<tr><td>${i.name} x ${i.qty}</td><td class="text-end fw-bold" style="color:var(--accent-color);">${formatVND(lt)}</td></tr>`;
      });
  ...
  $("#registerForm").submit(function (e) {
    ...
    setTimeout(() => {
      ...
      if (!/^0\d{9}$/.test(phone))
        errors.push("Số điện thoại phải gồm 10 chữ số và bắt đầu bằng 0.");
      ...
      if (users.some((u) => u.email === email)) {
        showNotification(
          "error",
          "Email Đã Tồn Tại",
          "Tài khoản với email này đã được đăng ký.",
        );
        btn.prop("disabled", false).html(orig);
        return;
      }
      ...
      showNotification(
        "success",
        "Đăng Ký Thành Công!",
        "Vui lòng xem lại thông tin bên dưới.",
      );
  ...
  $("#loginForm").submit(function (e) {
    ...
    setTimeout(() => {
      ...
      if (!email || !pwd) {
        showNotification("error", "Lỗi", "Vui lòng nhập email và mật khẩu.");
        btn.prop("disabled", false).html(orig);
        return;
      }
      let users = JSON.parse(localStorage.getItem("cakeShopUsers")) || [];
  ...
  if (currentPath === "profile.html") {
    ...
    $("#displayUserEmail").text(user.email);

    if (user.gender) {
      $(`input[name="profileGender"][value="${user.gender}"]`).prop(
        "checked",
        true,
      );
    }
    ...
    window.handleProfileSave = function () {
      ...
      user.phone = $("#profilePhone").val();
      user.dob = $("#profileDob").val();
      user.address = $("#profileAddress").val();
  ...
  window.renderReviews = function (productName) {
    let reviews =
      JSON.parse(localStorage.getItem(`reviews_${productName}`)) || [];
    let $list = $("#reviewsList");
    if (!$list.length) return;
    if (reviews.length === 0) {
      $list.html(
        '<p class="text-muted" id="noReviewsMsg">Chưa có đánh giá nào. Hãy là người đầu tiên!</p>',
      );
      return;
    }
    $list.empty();
  ...
  if (currentPath.includes("sanpham")) {
    ...
    $("#reviewForm").submit(function (e) {
      ...
      $("#reviewerName, #reviewComment").val("");
...
(function () {
  ...
  $("#addToCartBtn").on("click", function () {
    ...
    setTimeout(function () {
      $("#cartNavContainer").removeClass("animated-shake");
    }, 700);
  });
    ...
    $("#reviewStars i")
      .on("mouseenter", function () {
        ...
        $("#reviewStars i").each(function () {
          ...
          $(this)
            .toggleClass("bi-star-fill", isActive)
            .toggleClass("bi-star", !isActive);
        });
      })
